...
func runOperator(ctx context.Context, eventController *controllers.WatcherReconciler) {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		Scheme:             scheme,
		MetricsBindAddress: "0", // Desactivar las métricas del controlador
		Port:               9443,
		LeaderElection:     true,
		LeaderElectionID:   "eventd-operator-leader-election",
	})
	if err != nil {
		fmt.Printf("Error al crear el administrador del controlador: %v", err)
		os.Exit(1)
	}

	// Configurar el controlador del evento
	err = eventController.SetupWithManager(mgr)
	if err != nil {
		fmt.Printf("Error al configurar el controlador del evento: %v", err)
		os.Exit(1)
	}
